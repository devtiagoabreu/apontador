...
| Método | Rota | Descrição |
|---|---|---|
| GET | `/api/agendamentos-manutencao` | Lista (filtros `status`, `maquinaId`; sem `status` = todos; `limit`/`offset`) |
...
- **Fase 5 — Dashboard de manutenção** ✅ *concluída (2026-09-17)*: `/dashboard/manutencao` (visão geral: cards Agendados Hoje / Atrasados / Pendentes / Em Andamento + tabelas em andamento e últimos concluídos, botões para agendamentos e histórico); `/dashboard/manutencao/agendamentos` (lista com filtros status/máquina/data + detalhe via dialog + Cancelar com motivo opcional via `POST /api/agendamentos-manutencao/[id]/cancelar`); `/dashboard/manutencao/historico` (tabela completa com detalhe via dialog); API de agendamentos aceita ausência de `status` como "todos"; nav admin com grupo visual "Manutenção" (5 itens: visão geral, agendamentos, histórico, tipos, atividades); `tsc --noEmit` limpo, 108/108 testes, lint ok.
- **Fase 6 — Testes e validação** ✅ *concluída (2026-09-17)*: lógica de negócio extraída para `src/lib/manutencao.ts` (conflitos RF9 de início/finalizar/cancelar, schema de finalizar/reagendar, validação de `dataPrevista`, labels) consumida pelas rotas (`POST /api/manutencoes`, `POST /api/manutencoes/[id]/finalizar`, `POST /api/agendamentos-manutencao/[id]/iniciar`, `POST /api/agendamentos-manutencao/[id]/cancelar`); **23 testes unitários novos** em `src/lib/manutencao.test.ts` (total 131/131); `tsc --noEmit` limpo (campo `ativo` tipado como `boolean | null`), lint ok, `next build` compila e passa lint/type-check (a geração de páginas estáticas estoura memória no ambiente local — limitação da máquina, não do código); docs e glossário atualizados (termos: reagendamento, `origemManutencaoId`, `agendamentoId`, conflito RF9).
